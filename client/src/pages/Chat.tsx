...
const Chat = () => {
  ...
  const fetchMatchDetails = async () => {
    try {
      if (!hasValidCredentials()) {
        console.error('🔍 Chat - No valid credentials found');
        toast.error('Authentication required. Please log in again.');
        return;
      }
      
      const headers = getAuthHeaders();
      console.log('🔍 Chat - Fetching match details for:', matchId);
  ...
  const fetchChatHistory = async () => {
    try {
      ...
      const headers = getAuthHeaders();
      console.log('🔍 Chat - Fetching chat history for match:', matchId);
      
      const response = await fetch(`${config.API_URL}/api/messages/match/${matchId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          ...headers
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-orange-50">
        <header className="bg-white shadow-sm border-b border-gray-200">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Match Not Found</h1>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <Button onClick={() => navigate('/matches')} className="bg-gradient-to-r from-pink-500 to-red-500 hover:from-pink-600 hover:to-red-600 text-white">
              Back to Matches
            </Button>
          </div>
        </div>
